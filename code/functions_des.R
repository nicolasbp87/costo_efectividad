...
#' @param correction A character string indicating the type of correction to apply
#' to the sampled time to events. It can be either "none" or "uniform".
#' If "none", the sampled time to events will be the categories corresponding
#' to the maximum cumulative probability that is greater than or equal to a
#' uniform random number. If "uniform", the sampled time to events will be
#' the categories corresponding to the maximum cumulative probability that is
#' greater than or equal to a uniform random number plus a uniform random
#' number.
#' @details The function uses the cumulative sum of the probabilities in each
#' row to determine the category for each individual. It generates a uniform
#' random number for each individual and finds the first category whose
#' cumulative probability is greater than or equal to that random number. The 
#' function checks if the rows of `m_probs` sum to 1. If not, it normalizes 
#' those rows.
#' If the `correction` argument is set to "uniform", the function adds a uniform
#' random number to the sampled category, effectively shifting the time to
#' event uniformly within the range of the categories.
#' @examples
#' # Define a matrix of probabilities
#' m_probs <- matrix(c(0.2, 0.3, 0.5,
#'                     0.1, 0.4, 0.5,
#'                     0.3, 0.3, 0.4),
#'                   nrow = 3, byrow = TRUE)
#' # Sample time to events without correction
#' sampled_time <- nps_nhppp(m_probs)
#' # Sample time to events with uniform correction
#' sampled_time_corrected <- nps_nhppp(m_probs, correction = "uniform")
#' @references
#' Garibay D, Jalal H, Alarid-Escudero F. A Fast Nonparametric Sampling Method 
#' for Time to Event in Individual-Level Simulation Models. Medical Decision 
#' Making. 2025;45:205–13. 
