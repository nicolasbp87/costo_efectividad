...
TotPop <- function(time, prev, incid, disc = 0){
  # Computes the total population affected by the technology
  #
  # Args:
  ...
  LT            <- time[length(time)]
  # Vector with population affected by the technology at each time point
  pop.time      <- c(prev, rep(incid, (length(time) - 1))) 
  # Vector with present value of population affected by the technology at each time point
  disc.pop.time <- pop.time/(1 + disc)^time
  # Total population affected by the technology
